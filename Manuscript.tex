...
Given an historical log of defects seen previously in the code, XTREE deprioritizes bad smells that have little or no support in that log. 

When XTREE's recommendations are compared to 
  those found by previous work (Shatnawi, Alves, and Borges.)
...
are consistent with other research suggesting that bad
smells may not be a useful way to guide refactoring~\cite{Mantyla2004,Yamashita2013,Sjoberg2013}.
...
since they are not justifiedable  
with respect to the history of this project.
...
bad smell X or Y is most important. 
 XTREE offers a way to assess
 competing claims about different refactorings.
  

{\em PROBLEM \#2:} Not all bad smells can be fixed. 
Consider
a bad smell that is triggering on some very long method,
...
To address this second
  problem, we let
XTREE {\em reason optimistically} to find the set
$R_0$ of refactorings that might be useful
(for details on how that optimistic reasoning
is implemented, see \tion{rq}).
Since that set was generated optimistically,
the space of actually usable refactorings is 
the possibly smaller subset $R_1 \subseteq R_0$.
If the developer's proposed refactoring $R_2$ is
not in $R_0$, then it cannot be in the smaller
set $R_1$.  As shown
in this paper's experiments,  XTREE's
recommendations typically include only a small subset of
the code metrics. That is, we
can still rule out some proposed refactorings
since many of those proposals fall outside of the space
of refactorings suggested by XTREE. 
...
means that some section of code starts to smell bad.  
In all our  reading of the recent literature,
the only work we can find from other researchers that
offers metrics {\em and} thresholds come
from Shatnami~\cite{Shatnawi10}
and  Alves et al.~\cite{Alves2010} (for details on those
methods, see \tion{methods}).
One potential problem with their approaches is that 
they do not explore the interconnections between sets
of code metrics. This is important if sets of
...
{\em conjunctions} of metric+thresholds pairs that need to be changed {\em togehter}.
Yet Shatnami~\cite{Shatnawi10}
and Alves et al.   report   {\em disjunctions} of multiple metric/thresholds~\cite{Shatnawi10,Alves2010}
...
\item Another, completely separate, system to assess
how defective is the code before and after some
refactoring.
\end{enumerate}
For the second system,
 Cheng, O'Keefe, Moghadam and  Mkaouer et al. use the QMOOD hierarchical
quality model~\cite{Bansiya02}, constructed in 2002.
Our problem with using tools like QMOOD
...
that are specific to the systems   studied here.
  
 
 As to XTREE's limitations, the tool 
  can only comment
...
then a XTREE-style analysis must   wait till 
data about those new
terms has been collected.

Another  aspect to note about XTREE  is that the tool assumes
Fowler's bad smells
should be assessed only via their contribution to software defects
...
Scott-Knott test described above. In this example table, XTREE is ranked the best, while Shatnami is  ranked   worst.
...
entries show net effects of applying (seperately) all the disjuncts
proposed by those methods.

Three data sets are very responsive to defect reduction suggestions:
Jedit, Ant, and Ivy (all of which show best case improvements over 50\%.
 The Jedit results are  
surprisingly uniform  suggesting that in that data set that
our methods all find the same ways to reduce the expected number of
defects.  
...
\begin{figure*} 
\centering
\includegraphics[width=\linewidth]{figs/changes0.png}
...
% \input{tex/results_shatnawi.tex}
% \input{tex/results_alves.tex}
