...
Due to the continuous change undergone by modern software systems, developers tend to ignore the quality of source code, thereby introducing \textit{code smells} (or \textit{anti-pattern}). These anti-patterns are generally viewed as poor solutions to recurring design problems. In addition to there being anecdotal evidence of the impact of anti-patterns, there now is also some empirical evidence to suggest that code containing anti-patterns are more fault prone than the rest of the code~\cite{}\cite{}. Studies show that the number of code smells increase over time and in only a very few cases are these smells addressed by refactoring \cite{}\cite{}. This makes early detection of these code smells a necessity to maintain sufficient quality in software systems. This being said, detection only addresses a part of the problem, what we need are recommendation systems that can assist a software developed by (i) identifying potential anti-patterns; (ii) generating reliable refactoring options, and (iii) critiquing bad refactoring options. 

This is quite similar to the concerns voiced by several business users, who are demanding tools that support business-level interpretations of their data. At a panel on software analytics at ICSE`12, industrial practitioners lamented the state of the art in software analytics~\cite{menzies12a}. Panelists commented  ``prediction is all well and good, but what about decision making?''. Note that these panelists were more interested in the interpretations and follow-up that occurs after the mining, rather than just the mining itself. So:

\bi
\item
Instead of just \textit{detecting} anti-patterns and code smells, it may be beneficial to generate \textit{plans} to remove the bad smells
\ei
In response to this , we propose a novel {\em planning} method called XTREE for learning changes to a software system such that its performance ``improves'', according to some quantitative measure. This paper uses XTREE to reduce  the expected value of the defects in Jureczko et al.'s JAVA systems~\cite{jureczko10}; 

The contributions of this paper are (1)  the new XTREE algorithm and (2) an evaluation strategy that shows the algorithm performing significantly ``better'' than  the  planner  proposed in our prior   work~\cite{me12c}. In this paper, we focus on three research questions:

\bi
\item \textbf{RQ1: Effective: Can a planner suggest plans to improve the quality of a defective class?}
\item \textbf{RQ2: Succinct: Are the plans suggested by a planner succinct?}
\item \textbf{RQ3: Insightful: Do the plans tell us thing we do not expect?}
\ei
