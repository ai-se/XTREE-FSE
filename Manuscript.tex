...
\title{How to Learn Useful Changes to Software Projects\\(to Assist Refactoring)}
\numberofauthors{4} 
\author{  
\alignauthor
...
When modern software systems are developed, their design changes continuously. Due to market constraints, the developers tend to ignore the quality of source code, thereby introducing \textit{code smells} (or \textit{anti-pattern}). These anti-patterns are generally viewed as poor solutions to recurring design problems, possibly deterring the maintainability of the system. In the past decade there has been growing evidence of the impact of anti-patterns, there now is also some empirical evidence to suggest that code containing anti-patterns are more fault prone than the rest of the code~\cite{}\cite{}\cite{}. Studies show that the number of code smells increase over time and in only a very few cases are these smells addressed by refactoring \cite{}\cite{}. This makes early detection of these code smells a necessity to maintain sufficient quality in software systems. This being said, detection only addresses a part of the problem, what we need are recommendation systems that can assist a software developed by (i) identifying potential anti-patterns; (ii) generating reliable refactoring options, and (iii) critiquing bad refactoring options. 

Tools for detecting code smells have been the focus of several researchers lately, \cite{}. One popular analysis technique is the use of static code analysis and code quality metrics\cite{}. While these tools are capable of detecting these smells with a reasonable accuracy, they fail to provide developers in planning actions to improve the quality of source code.  Similar to the concerns were voiced by several business users, who are demanding tools that support business-level interpretations of their data. 

At a panel on software analytics at ICSE`12, industrial practitioners lamented the state of the art in software analytics~\cite{menzies12a}. Panelists commented  ``prediction is all well and good, but what about decision making?''. The panelists were more interested in the interpretations and follow-up that occurs after the mining, rather than just the mining itself. We call such tools \textit{planners}. In the context of detecting code smells, the planners are designed to address the following issues.
...
\bi
\item \textbf{RQ1:} \textit{Can a planner suggest plans to improve the quality of a defective class?} This research question investigates the effectiveness of a planner in suggesting useful refactoring options to improve the quality of class by reduce the number of defects. Specifically, we use XTREE to suggest valid changes to source code and we use an oracle to measure the effectiveness of the changes, see \tion{eval}.
\item \textbf{RQ2:} \textit{Are the plans suggested by a planner succinct?} The second question aims to measure the number of refactorings required 
\item \textbf{RQ3:} \textit{Insightful: Do the plans tell us thing we do not expect?}
\ei
...
Anti-patterns are poor solutions and recurring design problems that are unwillingly introduced by developers while designing and implementing large object-oriented systems. The impact of these code smells have been subjected to several empirical studies. As a result of the, the identification of these anti patterns has been receiving more attention from both researchers and practitioners.

During lifecycle, a software system undergoes continuous changes in order to keep up with the 
business level demands \cite{36}. Unfortunately, due to market/customers constraints, the source code quality is often neglected with the risk of introducing \textit{code bad smell} (or code-smells) \cite{24}. 

Fowler \cite{24} and Brown et al. \cite{13} defined a catalogue of more than 30 anti-patterns. For each anti-pattern, they reported the definition of the anti-pattern as well as specific refactoring operations aimed at removing it in order to improve the quality of source code. 

To illustrate this, consider for example developers adding several responsibilities to a class rather that including them in separate classes. This results in a class that grows rapidly, and when there are further added responsibilities, the classes grow and breed. Ultimately, the class becomes too complex and its quality deteriorates. Such classes are know as Large Class \cite{24} and could be the cause of a design problem in source code. 

For a while now, there have been anecdotal evidence that design problems (such as anti-patterns) negatively impacts software comprehension and maintenance. However, in the past few years, anti-patterns have been the subject of several empirical studies aiming at empirically analyzing their impact on software maintainability. From these studies, we now have empirical evidence that code containing code smells or participating in anti-patterns is significantly more change-prone than ``clean'' code \cite{31}. Also, code participating in anti-patterns has a higher fault-proneness than the rest of the system code \cite{31}\cite{37}. 

Other studies aimed at understanding the impact of anti-patterns on program comprehension \cite{1} showed that the presence of an anti-pattern in the source code does not decrease the developers' performance, while a combination of anti-patterns results in a significant decrease of their performance \cite{1}\cite{61}. While the results of this study indicates that single anti-patterns are not harmful, they also reveals that anti-patterns are quite diffuse in software systems and very often code components are affected by more than one anti-pattern. In addition, we have empirical evidence that the number of anti-patterns in software systems increases over time and only in few cases they are removed through refactoring operations \cite{3}\cite{17}. 

All these findings suggest that code smells and anti-pattern need to be carefully detected and
monitored and, whenever necessary, refactoring operations should be planned and performed to deal with them. Unfortunately, the identification and the correction of design flaws in large and non-trivial software systems can be very challenging. These observations call for \textit{planners} capable of supporting the software engineer by (i) identifying anti-patterns, and (ii) designing and applying a refactoring solution to remove them. Such planners, are the focus of the current paper.
...
Additionally, if a developer were to suggest a specific refactoring option, our planners can query the option and report on its possible usability. Note that we make no assumption that a plan mentions every feature
(so plan1 can be  more succinct than plan2 when plan1  mentions fewer features than
plan2).


\subsection{From Detection to Planning}\label{sect:p2p}
 
 
This paper is about the next step {\em after} detection of bad smells. Suppose developers are presented with  a list of anti-patterns and they do not like what they see. This user may then ask a {\em planning}  question; i.e. ``what can we change to do better than that?''.
...
person for their opinion on the impact of these anti-patterns and the possible refactoring options. Most advices offered by these developers are based on experiences, sometimes
such advice is an effective strategy and sometimes it is not.
...
The Jureczko data records the number of known defects for each class using a post-release bug tracking system. The classes are described in terms of nearly two dozen metrics, included in the Chidamber and Kemerer metric suite, such as number of children (noc), lines of code (loc), etc. For details on the Jureczko data, see  \fig{ck} and \fig{jd}. All the planning methods of this paper reflect on the numeric value of the raw defect counts. The predictor considers defects to be a Boolean class data set, where defects are \texttt{TRUE} if the numeric defect count is greater than zero and \texttt{FALSE} otherwise. The choice of this data set was is based on the taxonomy of the smells, in which we focus on the measurable properties needed to detect a given smell. This set of metrics is not restricted and can be easily extended with other metrics.
...
that find   plans   particular to each test case; and (2)~they  are {\em trust-increasing}; 
  i.e. the changed examples are moved {\em closer} to the training data.
...
continuous/discrete feature. Then, it
...
One potential problem with Method 1 is the {\em unsupervised} nature of
...
%   Regressor (from the SciKit learn kit~\cite{Pedregosa2012})   on one half, then applied to the other. After repeating this process 40 times, we achieved nearly perfect accuracy of $s=$\{98.45\%, 99.62\%, 94.86\%, 98.89\%, 98.60\%, 99.87\%\};
...
%  & CBO             & LCOM           & LOC & AMC & WMC & NPM    & CAM & RFC         & CA & AVG\_CC \\
%  & 4               & 3              & 3   & 3   & 2   & 2      & 2   & 2           & 1  & 1       \\
%  & Max: 9 \cite\{} & Min: 0, Max: 2 & *   & *   & 20  & Min: 1 & *   & 40 \cite\{} & *  & *      
% \end{tabular}
% \end{figure*}

% \includegraphics[width=\linewidth]{figs/ant.png}

\begin{figure*}[htbp!]
\centering
\includegraphics[width=\linewidth]{figs/changes.png}
\caption{Suggested Changes}
\label{figs:changes}
\end{figure*}

\subsubsection{RQ1: Effectiveness: Can plans to improve the quality of a defective class?}
...
\subsubsection{RQ2: Succinctness: Are the plans suggested by a planner succinct?}
...
\subsubsection{RQ3: Do the plans tell us thing we do not expect?}\label{sect:surprise}
