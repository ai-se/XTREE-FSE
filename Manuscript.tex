...
\title{ How to Recognize Useless ``Bad Smells''}
\numberofauthors{2} 
\author{  
\alignauthor
...
disagree on what bad smells should be refactored.
While it useful to fix  some bad smells, others are
 can be ignored.
How to distinguish useful from useless bad smells?

We propose RANK, a tool that judges how badly
a project needs a particular   refactoring.
RANK is tuned using the historical data of the current project.
Using RANK, we   show that the range of truly bad
``bad smells'' changes from project to project. Hence,
before refactoring, it is important to run a critique tool
like RANK to stop work on useless refactorings.
Further, using RANK, it is possible to show that many
...
guiding refactoring via bad smells. However,  
it is vital to RANK the bad smells defined for a 
particular project.
...
\noindent
{\bf Keywords:} Bad smells, refactoring,
performance prediction,  decision trees 
\end{abstract}

\section{Introduction}
\begin{raggedleft}
{\em My offense is rank, it smells to heaven.}\\
...
According to   Fowler~\cite{fowler99}, code smells (a.k.a. bad smells)
are ``a surface indication that usually corresponds to a deeper problem''.
Much research enthusiastically endorses ``bad smell''-based refactoring, as witnessed
by the  
recent literature of Tufano et al. at ASE'15~\cite{Tufano2015} which
lists dozens of  
papers proposing bad smell detection and repair tools.

Should we remove bad smells whenever we find them? Perhaps not.


Firstly, as noted by Shantnawi~\cite{Shatnawi10} there are  there are few studies that formulate 
precise guidelines,   represented as threshold values, to interpret the complexity of the software design using metrics. For example,  Darcy and Kemerer~\cite{darcy05}
discuss what OO design metrics might defect bad smells (e.g. a excessive
depth of inheritance tree) but they never comment at what point
a particular code measurement turns from ``ok'' to ``bad smell''.


Secondly, it is not clear that all code smells are relevant to
all projects.
Developers often disagree on what bad smells, if any at all,
...
Mantyla et al.~\cite{Mantyla2004} found that within an organization,
 ...
 How do we reconcile the enthusiasm of Fower,  Tufano, Darcy, Kemerer et al.
 with the more pessimism of Mantyla, Yamashita, 
 and Sj\"oberg et al.? The starting
 point of this paper is a comment
 from Mantyla et al.~\cite{Mantyla2004}. They say that
 when making  refactoring decisions, it is not enough to
 just apply some bad smell rule generated from another project.
 Rather, they say, decisions about refactoring
 should be based based on a combination of (1)~subjective evaluations and 
 (2)~code metrics.  
 
 Given the nature of human
 expertise, developers will offer many  strong opinions about what requires changing.  That is, we   anticipate no shortage of (1)~subjective
 evaluations from developers.
 
 As to lessons learned from (2)~code metrics, this paper introduces RANK,
 a tool for sorting proposed changes to static code measures. Unlike
 normal classification algorithms, RANK is a {\em contrast set learner}.
  Contrast set learners do not report {\em what is}. Rather
     they recommend {\em what to change} in order to move from
...
i.e. a code refactoring. 
 
RANK's concept of operation is that developers {\em propose}
numerous refactorings and RANK {\em disposes} of all those with
no support in history data.
Experiments with RANK explain the pessimism of Mantyla, Yamashita, 
 and Sj\"oberg et al. As they noted, sometimes, pre-defined bad
 smells are irrelevant to reducing the risk of defects in a project.
 However, when bad smells are learned from the current project,
 it is possible to recognize what are useful, and useless, local
 changes (where ``useful'' is defined in terms of reducing the
 predicted number of defects).
  Hence,   we  can endorse
 the use of bad smells to guide refactorings, but
 only when tools like RANK can  tune the definitions of
 ``good'' or ``bad'' refactorings using current project data.
...
Before beginning, we make the obvious aside
that defect predictors are not the {\em only} way to assess
bad smells. Some researchers report that code reviews and debating potential refactorings are useful for more just reducing defects. For example, Bosu and Carver~\cite{bosu13} report that
...
even if it does not address issues of defects. That said,
reducing the risk of errors was
Fowler's original motivation for introducing refactoring. Also, using
defect predictors,
it is possible to operationalize this test for $R_i \succ R_j$
while it might be harder to otherwise assess refactorings.
Hence, 
this paper assumes that {\em better} refactorings and
bettering at {\em reducing} the chances
that code will contain defects. 

\section{Motivation}\label{sect:prelim}

A frequently asked question about this work is 
why build tools like RANK to second-guess and critique developer intuitions?
Our reply is that  recent studies point to a  widespread malaise in software engineering:
...
To be fair, software engineering is not the only field with this problem.
...
The first three come from the general SE literature while the rest
...
software\footnote{171 software projects shepherded
by the SEI, 2006-2014} possibly this has been  heavily mitigated by $21^{st}$ software development languages, tools and development practices). 
Whatever the reason, the main point here is that phase delay remains a strongly
...
\input{tex/refactor}
 ...
 If widely-held beliefs in SE can be inaccurate and misleading,
 it is timely to check the utility of bad smells.
 This check is important since
 the concept of bad smells is highly pervasive in the SE literature\footnote{.
As of  Feburary 2016, ACM Digital Portal reports 
that Fowler's 1999 text has 6434 references;
while a search  on "bad smell"+refactoring yields 824 citations in
the last decade at learning research
venues (ICSE, FSE, etc).}. Accordingly,
\fig{smells} 
lists the commonly mentioned bad smells, in the first column.
The other
columns show other work that comments on which bad smells matter most.
A  blank cell indicates where the other work has chosen to ignore
of the Fowler bad smells as unimportant. 
Note that most of the cells are blank.
For example, the Atwood'06~\cite{Atwood06} study takes the time to comment on   
all the Fowler bad smells.
However, all the  other  studies ignore the majority of the Fowler bad smells.
 
Two of the studies in \fig{smells} offers some comments on the relative importance
of the different bad smells:
\begin{itemize}
\item The Yamashita’13~\cite{Yamashita2013} survey comments on the relative importance
    of 14 code smells-- only eight of which are mentioned in
    the left-hand-column.
  \item  
   The {\em developer survey}   (in the right-hand-side column}
   shows the opinions we collected from a dozen developers from a Washington
    D.C. web-tools development organization
...
\end{itemize} 
    One interesting point of agreement of the  Yamashita’13 study and the 
    developer survey is the agreement on which  bad smells
    were high priority:
    \begin{itemize}
        \item Three of the bad smells listed in the top half of the Yamashita'13 rankings also score very high in the developer survey.
        \item Those three were {\em duplicated code, large class}, 
        and {\em long method}.
        \ei
...
Human developers can be clever, but their thinking can also be flawed.
Hence it is important to double check their beliefs in, for example,
bad smells. Tools like HISTORY perform that check, using models learned
...
\subsection{Correlation, Causation, and Critique}

We propose HISTORY as way to critique (and possibly dismiss) proposed
refactorings. This section discusses the value of that approach.

HISTORY is a statistical method for finding ways to change code
such that the new code has a lower chance of having defects. 
To make these recommendations,
HISTORY  inputs frequency counts of how often a static
code attribute is seen in defective or non-defective modules.
Hence, its recommendations about what to change may be wrong
due to the famous correlation vs causation problem.
Statistical methods that use frequency counts can only see how often
certain events occur together, but not why that happens. Hence,
they may conclude incorrectly that with two co-located situations,
one might cause the other. This is not necessarily true. For
example, the number of storks and the number of births
in Denmark is not causally
connected. Nevertheless, both may be correlated if certain
migratory patterns of storks coincide with certain sociological factors.

Another reason that HISTORY's recommendations might be incorrect
is the presence of hidden constraints that are not apparent in
its input data. 
