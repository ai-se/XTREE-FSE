...
After that,	  find the {\em desired} leaf:
	 ...
	 for discrete attributes, delta is the value from {\em desired};
	   for  numerics, delta is the numeric difference;
	   and  for numerics  discretized into ranges, delta is a random number selected from the low and high boundaries of the that range. 
	     The bad smell threshold for   is then the delta that would move
	     the test case from the current to desired leaf.
	       
	     
\section{Evaluation}
\subsection{Research Questions}

Theoretically, XTREE is a good way to learn bad smell thresholds:
\begin{itemize}
    \item Unlike the outlier approach, XTREE offers bad smell
    thresholds that are specific to where test cases fall into
    the leaves of a decision tree that predicts for defects.
    That is, while the outlier approach offers one threshold for all
    metrics, XTREE specializes those thresholds to particular regions of
    the code.
    \item
    Unlike the outlier approach, XTREE is a multivariant approach
    that  understands when several code
metrics should  be changed in unison. 
\item Unlike CD, it is less verbose. 
XTREE does not report all the deltas between
adjacent clusters. Rather, it just reports the deltas in the code
metrics that a decision tree learner has determined are the minimun
changes required to distinguish one grouping from another.
\end{itemize}
Both these claims need to be tested, which leads to the following research
questions.
