...
it is possible to use automatic means. We assume that
developers will change code until a bad smell detector stops triggering.
That is, if the bad smell detector is {\em loc} $>$ 100, they will
redistribute the functionality of a long method into several smaller ones.
Recalling the italics in the above quote, Fowler's criteria for successful bad smell removal is the extent
to which the reorganized code has fewer errors/defects.
Data miners can be used to predict the defects in the reorganized code
so, in this paper, we will use static code defect predictors to assess
bad smells. 

After making that assessment, this paper finds
that   state-of-the-art methods
(Shantnawi~\cite{Shatnawi10}  in TSE 2010;
and our own instance-based systems~\cite{me12c} from 2012) 
are less effective for bad smell assessment than a new
contrast set learning method called XTREE.  
Standard learners, such as the regression methods used by Shantnawi~\cite{Shatnawi10}
only comment on current distributions.  
...
are a better way to learn when code needs to be reorganized
than, say, the methods of Shantnawi~\cite{Shatnawi10}.



We recommend using XTREE as a judge to rank the value of a set
of bad smell detectors:
\begin{itemize}
...
truly causal.
That said, if someone else's proposed bad smell detector $S_{\mathit{bad}} $ falls {\em outside} of the space of
effects found  using XTREE's analysis, then we can say that XTREE found 
{\em neither} correlation {\em or}
causal evidence that $S_{\mathit{bad}} $ can be found in the historical record.  
...
should be assessed via their contribution to software defects.   Some researchers 
...
\begin{itemize}
    ...
    that can critique plans of what to change within a data set in order
    to improve outcomes.
    ...
    \item Using that learner and that oracle, we offer a method for avoid code reorganizations that are irrelevant to the task of reducing the risk of introducing defects.
    \item A resolution between proponents of bad smells (e.g. Fowler, Shantnawi, Tufano~\cite{fowler99,Shatnawi10,Tufano2015}) and those who find them not useful or confusing   (e.g.Mantyla, Sj\"oberg,Yamahista et al.~\cite{Sjoberg2013,Mantyla2004,Yamashita2013}). We say
...
bad smells. Tools like XTREE perform that check, using models learned
