...
\conferenceinfo{FSE}{'16 Seattle, Washington}
...
% How often does XTREE make recommendations that
% use more than one code metric? 
% For example, continuing the scenario described in the introduction,
%   if {\em loc} is reduced, does XTREE also recommend that some coupling measure increase?
...
definition, increase the coupling between objects. 
 ...
 % sorry. ur still working. backing off
 % is this next para your "lesson" bubble? note that the main
 % thing here is to celebrate as expected how sometimes
 %XTREE increases certain measures.

% I see.. I've tried to write about that below
...
In accordance to that test,  we
sort a list of  $l=40$ values of $R$ values found in  $ls=4$ different methods. 
Then, we split $l$ into sub-lists $m,n$ as to maximize the expected value of differences  in   observed performances before and after divisions. E.g. for lists $l,m,n$ of size $ls,ms,ns$ where $l=m\cup n$: \[E(\Delta)=\frac{ms}{ls}abs(m.\mu - l.\mu)^2 + \frac{ns}{ls}abs(n.\mu - l.\mu)^2\]
