...
columns,  metrics are not recommended if they fail the $p \le 0.05$ test of \tion{p}.
...
between branches.  The most important feature of this data is that in the XTREE column, the changes occur to only a small number of code metrics. That is, all the other methods are far more {\em verbose} than XTREE (where ``verbose'' is defined in \tion{disc}).}\label{fig:counts}
\end{figure*}